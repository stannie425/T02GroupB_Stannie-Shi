...
/**
 * interactive circles: 1, 2, 8
 */
const interactiveCircles = [1, 2, 8];
let circleRotation = {
  1: 0,
  2: 0,
  8: 0
...
let hoverCircleId = null;
let lockedCircleId = null;

//preload audio
function preload(){
  vinylSound = loadSound("assets/vinyl-loop.mp3");
}
...
setInterval(()=>{
  if(vinylSound && vinylSound.isPlaying()){
    const slowSpeed = 0.15;
    if (lockedCircleId && interactiveCircles.includes(lockedCircleId)) {
      circleRotation[lockedCircleId] += slowSpeed;
    }

    if (!lockedCircleId && hoverCircleId && interactiveCircles.includes(hoverCircleId)) {
      circleRotation[hoverCircleId] += slowSpeed;
    }
    redraw();
  }
}, 16);
...
class CircleArt {
  ...
  display(id = null) {
    ...
    if (id && interactiveCircles.includes(id)) {
      rotate(circleRotation[id]);
    }
...
function drawAllCircles() {
  const circles = [
    ...
    new CircleArt(530, 180, 0.6, drawCircle8), //id = 8
    new CircleArt(785, 80, 0.8, drawCircle4),
    new CircleArt(-45, 270, 0.9, drawCircle3),
    new CircleArt(154, 460, 1.4, drawCircle1), //id = 1
    new CircleArt(434, 460, 1.4, drawCircle2), //id = 2
...
function setupAudio(){
  if(!vinylSound) return;

  //insert a filter between the sound and output
  vinylFilter = new p5.Filter();
  vinylSound.disconnect();
  vinylSound.connect(vinylFilter);

  //set a safe default so the first interaction doesn't cause clicks
  vinylFilter.setType("lowpass");
  vinylFilter.freq(800);

  vinylSound.setVolume(0.7);
  audioReady = true;
}

//determine which circle ID is under mouse
function getCircleIdAt(x, y){
  const tests = [
    {id: 1, cx: 154, cy: 460, r: 100 * 1.4},
    {id: 2, cx: 434, cy: 460, r: 100 * 1.4},
    {id: 8, cx: 530, cy: 180, r: 250 * 0.6},
  ];

    for (const c of tests) {
    if (dist(x, y, c.cx, c.cy) <= c.r) return c.id;
  }
  return null;
}

//Audio filter depends on circle ID
function applyFilterProfile(id){
  if (!vinylFilter) return;
  if(id === 1){
    vinylFilter.setType("highpass");
    vinylFilter.freq(1500);
  }
  if (id === 2) {
    vinylFilter.setType("bandpass");
    vinylFilter.freq(900);
  }

  if (id === 8) {
    vinylFilter.setType("lowpass");
    vinylFilter.freq(500);
  }
}

//control the mouse to play
function mouseMoved(){
